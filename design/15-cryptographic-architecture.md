...
**Status:** **Locked** (2026-06-07). Full DOCS review **RATIFY-WITH-EDITS** folded: **E1** schema naming `encrypted_kek`→`encrypted_dek` across §4.1 (column **and** comment → "scope DEK wrapped by the scope KEK"), §4.2 key-hierarchy row, and §8.2 `ScopeKey.encryptedDek`; **E2** §3.8 `PayloadCipher` wiring stated precisely (composition-root bridge in `com.homesynapse.app` — neither `persistence` nor `config` references the other; zero new module edge). Locked alongside **AMD-86** (INV-PD-07 narrow + INV-PD-03 at-rest posture) and **AMD-87** (Expectation codec); on-disk watermark → **AMD-87**. Review return: `nexsys-hivemind/context/audits/2026-06-06_Doc15_AMD-86_DOCS_Review_Return.md`. The `[BLOCKING-for-M6-impl]` m2 counter-nonce-durability hazard (§6/§13.4/§16) is carried forward as an explicit **M6 Open Risk** (pm-handoff) so it survives this Lock.
