...
**Backup-key portability seam (A4.5 — recorded at AB-4, 2026-06-28; seam only; docs-currency note landed at the 2026-07-03 Doc 18 Lock pass).** The machine-local `.root-key` (§3.5) is install-local — a backup restored on a *different* machine cannot re-derive the scope KEKs/DEKs from it. A restorable backup therefore requires an **exportable recovery artifact** that carries (or re-wraps) the key material so a restored install can re-install the DEKs. The natural shape: an operator-held recovery secret (or a passphrase-wrapped export of the root key / per-scope DEKs) produced at backup time and consumed at restore time, kept **out of the event log**. AB-4 reserves this as a **seam only** — the export/import mechanics are the backup/restore WU, whose restore-completion gate is the rotate-DEK-on-restore contract above (AMD-94 §2.1). Spine record: `nexsys-hivemind/context/handoff/cross-agent-notes.md`, the AB-4 A4.5 entry. This paragraph mirrors into §6's recovery table by reference (root-key loss row).
