...
> **Amendment pointer — AMD-101 (Realization correction, RATIFIED 2026-09-13):** §3.4 gains the delivery-invariant paragraph after the subscriber-lifecycle list — every position matching a LIVE subscriber's filter is delivered exactly once, in position order, regardless of notification order; the notification is a wake hint, delivery is the read-forward from the subscriber's in-memory cursor, the persisted checkpoint is the durable floor; §9 gains `live_read_batch` and `live_idle_tick_ms`. Realized by BUS-ORDER-1 (core `a458a64`). The amendment: `design/amendments/AMD-101_Doc01-3.4_Realization-Correction_Notify-Is-A-Wake-Hint_Delivery-Is-The-Read-Forward.md`.
...
**Re-entrant write contention.** The EventPublisher is the sole writer to the domain event store (single-writer model, LTD-03 WAL). However, correctness-critical subscribers — specifically the State Projection and the Pending Command Ledger — produce derived events by calling `EventPublisher.publish()` during their event processing. This creates a re-entrant write pattern: the subscriber processes event N, calls `publish()` to append derived event N+1, then resumes processing. The EventPublisher submits each SQLite append to the Persistence Layer's platform thread write executor (LTD-03) and parks the calling virtual thread until the WAL commit completes. This prevents carrier thread pinning from sqlite-jdbc's `synchronized native` JNI methods. Re-entrant writes from projection subscribers (State Projection, Pending Command Ledger calling `publish()` during event processing) are serialized through the same write executor. Because the single-writer model guarantees sequential write access, re-entrant writes do not deadlock — they queue behind the current write and execute when the platform thread becomes available. The subscriber's virtual thread parks during the write, freeing its carrier for other work. The subscriber's event processing loop is sequential (not concurrent), which guarantees this ordering. The potential contention arises only if a second subscriber also calls `publish()` concurrently — the SQLite write lock serializes these calls, with the second caller blocking until the first completes. At MVP event rates (< 500 events/sec), this serialization adds negligible latency (< 1 ms per contended write).

**Contract preservation.** The EventPublisher.publish() behavioral contract is unchanged by this threading change. The caller blocks until persistence completes — it blocks on a CompletableFuture.join() rather than directly executing JNI, but the semantics are identical. The event is durable before the method returns. INV-ES-04 (Write-Ahead Persistence) is satisfied identically: the caller's virtual thread does not resume, and no subscriber is notified, until the WAL commit completes on the platform thread. The platform thread executor changes which thread executes the JNI call, not when the caller resumes or when subscribers are notified.
...
event_model:
  ...
  bus:
    ...
    live_read_batch: 64                 # Events per LIVE page read from the subscriber's cursor (AMD-101; EventBusConfig.liveReadBatch)
    live_idle_tick_ms: 250              # The LIVE loop's bounded park between page reads; caps a lost wake at one tick (AMD-101; EventBusConfig.liveIdleTick)
...
† All database operation latency targets include platform thread executor scheduling overhead (LTD-03). The executor adds an estimated 0.1–0.5 ms per operation due to the virtual-thread-to-platform-thread handoff. **Empirical validation (V3 spike, 2026-04-02):** measured overhead is p50=0.029 ms, p95=0.068 ms, p99=0.105 ms — below the low end of this estimate at all percentiles. The single-writer model (LTD-03) bounds write throughput to 1/(WAL commit latency) regardless of threading model — the writes were always serialized. On Pi 5 NVMe, C1 measured 50,964 events/sec in direct-call mode and V3 measured 24,473 events/sec through the executor. The throughput reduction in burst mode (~52%) is dominated by the executor handoff (~29 µs) being comparable to the DB operation (~19 µs) on fast NVMe; at production rates (100 events/sec sustained), the executor adds 0.29% overhead. The 500 events/sec target is validated with 49× headroom. If measured throughput falls below 400 events/sec on target hardware, investigate batched WAL commits (grouping multiple events into a single transaction).
...
This subsystem implements the `HealthContributor` interface (Doc 11 §8.1, §8.2) to report health state transitions to the HealthAggregator. The Event Bus is classified as CRITICAL_INFRASTRUCTURE tier (Doc 11 §7.1) with a 10-second startup grace period. Health state changes are reported via `HealthContributor.reportHealth(status, reason)` when the conditions in the table above transition. Note: the CRITICAL state in this subsystem maps to UNHEALTHY in the HealthAggregator's three-state model per Doc 11 §7.1.
