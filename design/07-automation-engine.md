...
**Dependents:** REST API (§8.1 AutomationRegistry/RunManager/PendingCommandLedger interfaces for automation CRUD, run trace, and command status endpoints per Doc 09 §3.2 and §7, §8.2 AutomationDefinition/PendingCommand types for response serialization per Doc 09 §4, §4.1 Run record for run trace query responses, §4.2 run trace assembly for causal chain rendering per Doc 09 §3.2), WebSocket API (live run trace streaming), Observability & Debugging (automation metrics, trace viewer data), Startup, Lifecycle & Shutdown (Doc 12 — Phase 3 automation initialization, §3.9 shutdown step 5, cascade governance)
...
2. Resolves the entity's owning integration via the two-hop `EntityRegistry.findEntity(entityRef)` → `Entity.deviceId()` → `DeviceRegistry.findDevice(...)` → integration (Doc 02 §8.1). _(AMD-95 currency: there is no `DeviceRegistry.getIntegrationForEntity`; the shipped `StandardCommandDispatchService` resolves via this two-hop path.)_
...
5. If routing succeeds, invokes the adapter's `CommandHandler.handle(CommandEnvelope)` callback (Doc 05 §3.8), where the entity reference, command name, and parameters are `CommandEnvelope` fields. _(AMD-95 currency: source is `void handle(CommandEnvelope) throws Exception`; `handleCommand(...)` does not exist.)_
...
**Event-driven, co-located for MVP (AMD-95 / §1 decision D1, RATIFIED 2026-06-25).** The dispatch shape above — the executor emits `command_issued` to the log and `command_dispatch_service` consumes it as a real bus subscriber — is the **ratified target**. The log is the single source of truth for dispatch (not merely its record): a subscriber acts only in LIVE, so replay-safety is structural (D2 pure-function-replay); the seam scales additively to cross-process/host/cloud/AI consumers; and AMD-31 ordering comes free from global position. For MVP the dispatch subscriber runs **in-process / co-located** with the engine — the seam is correct and scales later while latency is a single local hop (empirically ≈1.9 ns over a direct in-process call; the §1 V2-A Pi spike). **Current-vs-target currency note:** as-of M7.3 (`5363347`) the executor instead calls `CommandDispatchService.dispatch(...)` in-process and emits no `command_issued`, so the Pending Command Ledger has no live input yet; **M7.4a** wires the event-driven/co-located shape (the `command_issued` producer + `command_dispatch_service` as a co-located subscriber, with a paired `stop()` teardown), at which point this section's description is live. This supersedes the v5 "emit `command_issued` but keep dispatch in-process" hybrid lean.
...
1. A `command_issued` event arrives. The ledger records a pending entry keyed by `(target_ref, command_name)` with the `Expectation` resolved from the **target capability** — `EntityRegistry.findEntity(target_ref)` → the `CapabilityInstance` carrying the `command_type` → `CommandDefinition.expectedOutcomes()` (Doc 02) — together with the `confirmation_timeout_ms` and the `idempotency_class`. _(AMD-95 currency: `CommandIssuedEvent` is the frozen 5-component record and carries no expectation; `ExpectationFactory` is an interface with no implementation and is not on the live path. Confirmation tracking is gated by the capability's `ConfirmationPolicy.mode()`: `ConfirmationMode.DISABLED` ≡ optimistic / no ledger tracking; any other mode ≡ track-and-confirm — the M7.3 `ConfirmationPolicyGateTest` drives the bypass through this signal.)_
...
3. A `state_reported` event arrives for the target entity. The ledger coerces `StateReportedEvent.value` (a `String` at HEAD) to the typed `AttributeValue` the `Expectation` needs, directed by the expected `AttributeType` (total; an uncoercible value → `DegradedAttributeValue` typed-absent — the M7.2b precedent — never throws), then evaluates it against the pending entry's `Expectation` (via the `Expectation.evaluate()` interface from Doc 02 §8.2). If the result is `CONFIRMED`, the ledger produces a `state_confirmed` event referencing both the `command_issued` and `state_reported` events. _(AMD-95 currency: the `String`→`AttributeValue` coercion is removed if `state_reported` is later typed at the source — a future additive change.)_
...
record PendingCommand(
    ...
    Expectation expectation,     // from CommandDefinition.expectedOutcomes() (AMD-95; capability-sourced, not ExpectationFactory)
...
| Device Model & Capability System | Reads from | `EntityRegistry`, `CapabilityRegistry`, `DeviceRegistry`, `CommandValidator`, `CommandDefinition.expectedOutcomes()` query interfaces | Entity metadata, capability schemas, command validation, expected outcomes, entity-to-integration ownership | Read-only. Queries at trigger time (selector resolution), action time (command validation), and pending command creation (capability-sourced expectation generation — AMD-95; `ExpectationFactory` is interface-only, not on the live path). |
...
| Integration Runtime | Calls | `IntegrationContext.CommandHandler.handle(CommandEnvelope)` | A `CommandEnvelope` (entity reference, command name, parameters as fields) | Via Command Dispatch Service. The adapter's response determines `command_result` production. (AMD-95 currency: source is `handle(CommandEnvelope)`, not `handleCommand(...)`.) |
...
| D16 | Confirmation policy placement (AMD-95) | Confirmation is sourced from the **capability** (`ConfirmationPolicy.mode()` / `ConfirmationMode`, `DISABLED` ≡ optimistic), not from a `CommandAction` component; the `Expectation` is built from `CommandDefinition.expectedOutcomes()` | Matches the M7.2b-frozen 4-field `CommandAction` and the as-built ledger; supersedes-in-part AMD-90 §2.1 (the never-added 5th component + the automation-resident `ConfirmationPolicy{OPTIMISTIC,REQUIRED,BEST_EFFORT}` enum). No engine retry (AMD-90-INV-01) preserved. | §3.11.1, §3.11.2 |
...
| 2026-06-26 | Integrated AMD-95: Command-Pipeline Currency (event-driven/co-located dispatch + capability-sourced confirmation). §3.11.1 affirms the event-driven shape + co-located-for-MVP + current-vs-target note; corrects step 2 to the two-hop `EntityRegistry→DeviceRegistry` integration resolution and step 5 to `CommandHandler.handle(CommandEnvelope)`. §3.11.2 corrects the expectation to capability-sourced `CommandDefinition.expectedOutcomes()`, adds the `ConfirmationMode.DISABLED` gating + the `String`→`AttributeValue` coercion note. §4.3 record comment + §7 interaction table corrected. §16 D16 added. No event-type/module-info/source change (counts 71/41/53). | AMD-95 |
