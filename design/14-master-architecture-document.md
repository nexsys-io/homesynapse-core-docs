...
| Hybrid thread architecture for JNI-pinning I/O | Platform thread isolation is used in two subsystems: (1) the Integration Runtime isolates JNI-pinning serial I/O libraries (jSerialComm) on dedicated platform threads (Doc 05 §3.2), and (2) the Persistence Layer isolates JNI-pinning sqlite-jdbc operations on a bounded platform thread executor (LTD-03, Doc 04). In both cases, virtual threads submit work to platform threads and park until the result is available. This pattern prevents carrier thread starvation from `synchronized native` JNI methods. | Doc 05 §3.3, Doc 08 §3.2, Doc 04 |
