...
**Realization currency (M9.4a, 2026-07-04 — AMD-98 §2.4):** the `UnsupportedOperationException → Permanent` row is now enforced in `ExceptionClassifier` (it was previously documented here and in the enum javadoc but absent from the code — the classifier's default-to-transient swallowed it). Additionally, a `PermanentIntegrationException` anywhere in a cause chain now classifies PERMANENT (the PIE-only cause-walk, hop-capped): deliberately-thrown permanent intent is honored even when wrapped, while every other wrapped exception keeps the default-to-transient guard this section mandates. Three pinned tests hold all three directions.
...
The descriptor is the integration's contract with the supervisor, declared by the `IntegrationFactory` at discovery time. The `IntegrationDescriptor` declares the integration type (a string key like `"zigbee"`), not the instance identity. The supervisor assigns an `IntegrationId` (ULID, per LTD-04) when the integration is first loaded. This instance identity is stable across restarts and is the `integration_id` referenced in all scoped interfaces and event payloads. The distinction: `integrationType` identifies the software; `IntegrationId` identifies the installed instance. *(DP-B, ruled 2026-07-02 and confirmed 2026-07-03 — PERMANENT policy, noted at the Doc 18 Lock pass: the instance `IntegrationId` is **derived deterministically** — a restart- and reinstall-stable pure function of the integration type string, not a stored random assignment; no identity file exists to back up, corrupt, or crypto-shred. The rename hazard is a naming-layer concern Doc 18 §3.5's namespace governance defuses; M9.3 device adoption is identity-ungated. Implemented at M9.1.)*
