...
> **Amendment currency — AMD-52 (RATIFIED 2026-05-31), body FOLDED — M4.0b-4b shipped `72596cb` (2026-05-31).** `core/persistence` carries a custom `AttributeValue` `JsonSerializer`/`JsonDeserializer` pair (registered alongside `PersistenceJacksonModule` — the DECIDE-M2-03 pre-declared expansion point), emitting a compact `{"t":<AttributeType>,"v":…}` tagged-union envelope with an exhaustive no-`default` switch over the 8 `AttributeValue` variants, **no `@JsonTypeInfo`** (Jackson-isolation HARD RULE; enforced by ArchUnit `NO_JACKSON_IN_DOMAIN_MODEL`), and **no new `requires` or Jackson artifact** (`AttributeValue` reachable via `persistence → com.homesynapse.value`; `JsonSerializer` is in `jackson-databind`). Float identity is bit-anchored (`Double.doubleToLongBits`; stored text round-trippable, not byte-frozen); non-finite `FloatValue` uses JSON-valid sentinels (`"NaN"`/`"+Inf"`/`"-Inf"`); `chain_hash` stays the inert AMD-37 zero-reservation. `CheckpointSerializer` materializes `attributes` as a typed envelope per entry in the same `view_checkpoints.data` BLOB — **no row migration on `events` or `view_checkpoints`** (the typed payload stays in-BLOB; per-event `schema_version` is the discriminator). **Folded into §3.12 (typed checkpoint content).** See `design/amendments/AMD-52_*` + AMD-52-INV-02/03/04/06 (Architecture_Invariants_v1.md §22).
...
4. **Subscriber checkpoint safety check:** query `SELECT MIN(last_position) FROM subscriber_checkpoints WHERE subscriber_id IN (SELECT subscriber_id FROM subscriber_checkpoints WHERE last_position > 0)`. If any active subscriber's `last_position` falls within the deletion range, the behavior depends on the subscriber's activity:
   ...
   <!-- Added 2026-03-21: Architecture benchmark assessment finding S-2 -->
   - **Subscriber pause state.** A subscriber may be placed in PAUSED state via the REST API (e.g., during planned maintenance of an optional analytics module). A PAUSED subscriber's checkpoint is protected from retention indefinitely — the retention engine treats it identically to an active subscriber for checkpoint safety purposes. The PAUSED state is distinct from a stalled subscriber (one whose `last_updated` has exceeded the grace period). A PAUSED subscriber's `last_updated` is frozen at the time of pause and does not trigger the stalled-subscriber override. The maximum recommended pause duration is 7 days. After 7 days, the system produces a `subscriber_pause_warning` NORMAL event daily. After 30 days, the system produces a `subscriber_pause_critical` CRITICAL event warning that the subscriber's checkpoint protection is consuming retention capacity. The system never automatically un-pauses or overrides a PAUSED subscriber — only the REST API or a process restart can change the state.
...
<!-- Added 2026-03-21: Architecture benchmark assessment finding S-5 -->

**Known characteristic: duplicate-value domain event volume.** Sensors reporting at intervals below the telemetry threshold (e.g., temperature every 30 seconds) produce `state_reported` events in the domain event store even when the reported value matches the current canonical state (no `state_changed` is derived). These events are retained for the DIAGNOSTIC tier's retention period (default 7 days). At MVP scale (30 sensors × 2,880 events/day = 86,400 events/day, ~600K events at 7-day retention), this is well within SQLite's capacity on NVMe storage.

The routing threshold (> 1 sample per 10 seconds sustained) is a rate heuristic, not an information-value filter. A future optimization could tag duplicate-value `state_reported` events for accelerated retention (shorter than the standard DIAGNOSTIC period) or route them to the telemetry ring store. This would require a metadata tagging mechanism that does not violate event immutability (INV-ES-01) — for example, a retention-hint column in the events table that the retention engine consults but that is not part of the event's logical content. This optimization is not MVP scope but is recorded as a known scalability consideration for deployments exceeding 100 sensors.
...
**Typed checkpoint content (AMD-52, M4.0b-4b `72596cb`).** The `data` BLOB remains opaque to the Persistence Layer, but for the `"entity_state"` view `CheckpointSerializer` now materializes each attribute as a typed `AttributeValue` tagged-union envelope (`{"t":<AttributeType>,"v":…}`) rather than a flattened `String` — the same hand-rolled `AttributeValue` (de)serializer used for the `state_changed` event payload (registered alongside `PersistenceJacksonModule`; no `@JsonTypeInfo`; Jackson confined to `core/persistence`). The serializer preserves the `Include.ALWAYS` null round-trip (a schema-declared attribute that has never reported survives as a `null` value). **This is a content change only — no `view_checkpoints` row/column migration** (the typed envelope stays in-BLOB). A checkpoint that fails to deserialize falls through to clear-and-replay reconciliation, exactly as before; the `projectionVersion` 3→4 bump triggers that replay once on first boot.
