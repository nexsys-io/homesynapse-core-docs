...
**Dependents:** Persistence Layer (§3.12 view_checkpoints table, §8.1 CheckpointStore implementation, §3.2 same-transaction atomicity guarantee), Automation Engine (state queries for trigger evaluation and condition checking), REST API (§8.1 StateQueryService interface for current state and entity listing endpoints per Doc 09 §3.2, §4.1 EntityState record for response serialization per Doc 09 §4, §4.2 StateSnapshot for bulk state queries, §5 viewPosition contract for ETag consistency per Doc 09 §3.7), WebSocket API (§8.1 StateQueryService interface for include_initial_state snapshot delivery per Doc 10 §3.4 and §4.2, §4.1 EntityState record for state_snapshot message serialization per Doc 10 §4.2, §5 viewPosition contract for snapshot consistency tracking per Doc 10 §4.2), Observability & Debugging (Doc 11: §11 health indicator for health aggregation per Doc 11 §7.1, §8.1 StateQueryService for state freshness metrics per Doc 11 §3.5, §5 viewPosition contract for projection lag monitoring per Doc 11 §3.5), Web UI (dashboard state rendering, entity history graphs), Startup, Lifecycle & Shutdown (Doc 12 — Phase 3 State Store checkpoint recovery, projection versioning, reconciliation)
**Author:** HomeSynapse Core Architecture
**Date:** 2026-03-05

> **Amendment currency — AMD-52 (RATIFIED 2026-05-31), body FOLDED — M4.0b-4b shipped `72596cb` (2026-05-31).** AMD-52 ends the materialized-`StringValue`-only regime that AMD-51 §1.2 / AMD-51-INV-05 left in place: `ProductionDerivationRule` emits a **typed** `StateChangedEvent` (carrying the `AttributeValue` it already reconstructs for the AMD-51 comparison, at `schema_version = 2`), and `StateProjection.applyToState` + the AMD-50 backfill materialize the **typed** value into `EntityState.attributes` instead of `new StringValue(...)`. `shouldPublishDerived` is a typed-coherent comparison. This is a materialized-output change → `projectionVersion` **3→4**, riding AMD-50's frozen reconciliation-backfill unchanged (Path A re-derivation from the immutable `state_reported` log is authoritative). **Folded into §4.1 (`EntityState.attributes`); the §3.2 REPLAY/LIVE processing tables are mechanism-level and already version-neutral (`new_value` applies typed or String identically).** See `design/amendments/AMD-52_*` + AMD-52-INV-01/05/06/07 (Architecture_Invariants_v1.md §22).

> **Amendment currency — AMD-53 (RATIFIED 2026-05-31), implemented by M4.0b-5 (instruction issued).** This is a **code-compliance** amendment, **not a §4.1 contract change**: §4.1 already specifies `lastChanged`/`lastUpdated`/`lastReported` = "the `event_time` (falling back to `ingest_time`)…", but the LIVE `StateProjection.applyToState` code stamped all three from the projection wall-clock (`clock.instant()`), and entity-adoption seeding did the same. AMD-53 sources the three from `eventTime ?? ingestTime` in **every** projection path (LIVE all branches, reconciliation backfill, adoption seeding), making them replay-deterministic and bringing the code into compliance with the §4.1 text below — extending AMD-50-INV-03 from the `DerivationRule` to the materialization. `staleAfter`/`stale` are explicitly carved out and **stay real-time** (read-time `Instant.now()` vs `staleAfter`; §3.8 — AMD-53-INV-02). Materialized-output change → `projectionVersion` **4→5** on AMD-50's frozen backfill (heals legacy wall-clock timestamps on the upgrade boot). See `design/amendments/AMD-53_*` + AMD-53-INV-01/02 (Architecture_Invariants_v1.md §23).
...
| `attributes` | `Map<String, AttributeValue>` | `state_changed` events | Current value of each attribute, keyed by `attribute_key`. Attribute mutations are applied exclusively from `state_changed` events — never directly from `state_reported`. The `AttributeValue` sealed hierarchy lives in the `com.homesynapse.value` leaf module (relocated from device-model in M4.0b-4a; its type system is described in **Device Model & Capability System** §3.5). **Post-AMD-52 (M4.0b-4b, `72596cb`) the materialized value is the genuine typed variant** — `StateProjection.applyToState` and the AMD-50 reconciliation backfill write the typed `AttributeValue` carried in the `schema_version = 2` `state_changed` payload, ending the interim String-only materialization AMD-51 left in place. On a `projectionVersion` 3→4 reconciliation the typed value is reconstructed from the immutable `state_reported` log (Path A). Null values indicate an attribute that exists in the capability schema but has never received a report (preserved verbatim through the checkpoint round-trip). |
...
The health state is reported via the system health API (see **Observability & Debugging** doc 11) and is visible in the dashboard's system status panel. This subsystem implements the `HealthContributor` interface (Doc 11 §8.1, §8.2) to report health state transitions to the HealthAggregator. The State Store is classified as CRITICAL_INFRASTRUCTURE tier (Doc 11 §7.1) with a 60-second startup grace period (accommodating replay duration). Health state changes are reported via `HealthContributor.reportHealth(status, reason)` when the conditions in the table above transition. The STARTING state is unique to this subsystem — no other subsystem defines a startup-specific health state, but the State Store's replay phase produces legitimately stale data that consumers must be aware of.
