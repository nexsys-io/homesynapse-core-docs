...
**Transport layer (platform thread).** A single dedicated platform thread owns the serial port file descriptor via jSerialComm. This thread performs all `readBytes()` and `writeBytes()` calls, isolating JNI-induced carrier thread pinning from the virtual thread pool. The transport layer is responsible for byte-level framing: UNPI framing for ZNP (SOF detection, length extraction, XOR checksum validation), or ASH framing for EZSP (byte destuffing, data derandomization, CRC-CCITT validation, ACK/NAK generation). Parsed frames are placed on an inbound `BlockingQueue<ZigbeeFrame>`. Outbound frames are consumed from a write queue and serialized to bytes. *(Dependency currency — W6, corrected at M9.2, noted 2026-07-03: jSerialComm 2.11.2 ships an **explicit JPMS module** — `module-info.class` in the jar, consumed as a plain `requires com.fazecast.jSerialComm` — not an automatic module; catalog-pinned per LTD-10 with the Apache-2.0 license election recorded at the catalog line.)*
...
If the EZSP version negotiated during initialization falls below 13, the adapter logs a structured WARN entry (`zigbee.ezsp_legacy_version`) identifying the negotiated version and recommending a firmware update, then proceeds with best-effort operation. If the version falls below 8, the adapter throws a `PermanentIntegrationException` because the frame format is incompatible. This tiered approach avoids bricking existing setups while clearly communicating the supported path. **Band-edge policy above the band (R4 — ratified at the M9.2 closeout, noted 2026-07-03):** a negotiated version **above v14** is treated as the below-8 class — an unknown dialect, `PermanentIntegrationException`, never best-effort (misdecoding an uncharacterized frame dialect risks silent corruption; the band widens only by deliberate amendment after the new firmware line is characterized — the AMD-96 pattern). *(Transport election DP-C, ruled 2026-07-02: **EZSP-first** — the reference bench instrument is EZSP silicon (MG24, frozen at 7.4.5.0/EZSP v13); the full `ZnpTransport` implementation is Wave-2 (ZBDongle-P arrival), shipping probe-encode-only until then.)*
...
*(DP-D, ruled 2026-07-02 — noted 2026-07-03: the Tuya (§3.8) and Xiaomi/Aqara (§3.9) manufacturer codec **implementations are Wave-2-deferred** — no captured corpus exists for them yet (bench Wave-1 = Hue LCA017 + SNZB-03P); the scaffold types are reserved and these sections remain the design contract they build against. Per the M9.3 doctrine: freeze the schema, defer the behavior.)*
...
*(R5, ratified at the M9.2 closeout — noted 2026-07-03: a stored-parameters **resume mismatch** — the coordinator's live network state disagreeing with the persisted `NetworkParameterStore` record — is a `PermanentIntegrationException`, **never a silent re-formation**: silently re-forming a network orphans every paired device. The operator recovery story (fresh-network re-pair, DP-F) rehearses at the M9.4 bench acceptance.)*
