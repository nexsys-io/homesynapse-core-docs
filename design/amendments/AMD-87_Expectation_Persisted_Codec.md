...
source: M4.C deferral ([BLOCKING-for-M9] serde, Nick-arbitrated Option 2 2026-06-05); the executable acceptance test `EventPayloadCodecTest.CapabilityEvents.capabilityAdded_onOff_roundTrips` (`@Disabled("AMD-87 pending")`); AMD-52 AttributeValue tagged-union codec precedent
...
amd-number-rationale: reassigned from the retired AMD-65 → **AMD-87** per **INV-GA-02**. The pm-handoff ledger records AMD-64/AMD-65 as RETIRED post Research 5 v2 and "next active block starts at AMD-86"; AMD-86 took the INV-PD-07/crypto amendment, so AMD-87 is the next clean number (confirmed free). The Expectation codec was tracked as "~AMD-65" at M4.C (2026-06-05) before the retirement was reconciled. **NB:** the live acceptance test's annotation literally reads `@Disabled("AMD-65 pending")` at HEAD `8ef9e9f` — that is the retired number in the existing code; M5-A removes the annotation. Do not re-introduce AMD-65 anywhere.
-->
...
- [ ] Nick ratification.
- [ ] On ratification: register AMD-87-INV-01; raise the watermark to 65 (or fold in the M5-window mechanics); nav-index row.
