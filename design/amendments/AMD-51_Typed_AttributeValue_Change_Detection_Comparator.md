...
**Erratum (2026-05-31, M4.0b-3 closeout):** §2.6 gained a clause for the **schema-not-found** case (no `AttributeSchema` for the reported key) → `StringValue` string-compare fallback (NOT Degraded/no-emit) — deviation D-1, PM-adjudicated ACCEPT, independent review → APPROVE. See the erratum block in §2.6. Implementing WU **M4.0b-3 is COMPLETE + COMMITTED `98f705b`** (typed comparator shipped; `projectionVersion` 3; this amendment APPLIED).
...
- **Schema-not-found (no `AttributeSchema` for the reported `attributeKey`) → `StringValue` string-compare fallback** (see the erratum below).

> **Erratum (2026-05-31, applied at M4.0b-3 closeout — deviation D-1, PM-adjudicated ACCEPT; independent review → APPROVE).** §2.6 reconstructs by `AttributeSchema.type` and therefore presupposes a schema exists for the key; it did not specify the **no-schema** case. As implemented in `AttributeValueReconstructor` and ratified by this erratum: **when the resolver knows no schema for the `attributeKey`, both operands reconstruct as `StringValue` and the comparator performs an exact string compare — NOT a `DegradedAttributeValue`/no-emit.** Rationale: the materialized prior is itself always a `StringValue` (§1.2), so this is the natural identity and reproduces the pre-typed (M4.0b-2) behaviour for unschematized keys; treating no-schema as Degraded would *suppress every report forever*, permanently freezing an unschematized attribute — a worse regression than the phantom-change problem AMD-51 fixes. It also keeps the no-arg `DerivationRule.production()` gateway (empty resolver) behaviourally identical to the M4.0b-2 string rule, so the entire pre-existing test suite (all non-catalogue keys) stays green unchanged. In production the `StandardCapabilities` resolver covers all standard traffic, so this path is reached only for genuinely-unknown keys (logged at DEBUG). **This supersedes the M4.0b-3 coding-instruction error-table row "`schema` not found → Degraded / no-emit".** The Degraded path remains correct for a parse/reconstruction failure of a *schematised* value (the prior bullet) — only the no-schema case is the string-compare fallback.
