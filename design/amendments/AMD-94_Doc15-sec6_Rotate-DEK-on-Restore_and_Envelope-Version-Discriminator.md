...
status: **RATIFIED 2026-06-19** — authored by the Cowork PM (nexsys-project-manager); FULL independent DOCS-Project review **RATIFY-WITH-EDITS** (return `nexsys-hivemind/context/audits/2026-06-19_AMD-94_DOCS_Review_Return.md`); Nick co-signed the ratification with **E-1** (boot-invariant restore⇒rotation-only sharpening) + **E-2** (encoding-neutral discriminator) folded and the **§8.1 currency rider kept**. The §2.4 staged edits (A–F + G, E-1/E-2 applied) are folded into Locked Doc 15 (§3.4/§4.1/§5/§6/§13.4/§16/§8.1); `AMD-94-INV-01/02` registered (Architecture_Invariants_v1.md §48 + §17 + §18); on-disk watermark **AMD-93 → AMD-94** (invariants 165/48); §6 `[BLOCKING-for-M6-impl]` marker resolved; **OR-M6-NONCE restore-half → CLOSED**; AB-4 un-gated.
...
- [x] **FULL independent DOCS-Project review** (NOT the lightweight block-track — touches a persisted shape + a crypto behavioral contract + mints invariants; constraint-enforcement §6 / P4). Return: `nexsys-hivemind/context/audits/2026-06-19_AMD-94_DOCS_Review_Return.md` — **RATIFY-WITH-EDITS** (E-1 + E-2; §8.1 rider KEEP).
- [x] Nick ratification (co-signed 2026-06-19; the PM did not self-ratify). Nick independently verified E-1 (the boot-invariant restore-half false-pass) and the EDIT B anchor.
- [x] On ratification: folded the §2.4 staged edits (A–F + G, with E-1/E-2 applied) into Locked Doc 15 §3.4 / §4.1 / §5 / §6 / §13.4 / §16 / §8.1; resolved the §6 `[BLOCKING-for-M6-impl]` marker; flipped **OR-M6-NONCE restore-half -> CLOSED**.
- [x] On ratification: registered `AMD-94-INV-01/02` (INV-GA-02) into `Architecture_Invariants_v1.md` (§48 `AMD-94-INV` section + §17 index + §18 matrix rows + §0.3 prefix row); raised the on-disk watermark **AMD-93 -> AMD-94** (invariants 165/48).
- [ ] R-gamma return folds the version **policy** (registry / downgrade / AAD binding) — separate, later, non-blocking. **(Still open — the slot + `v1` are reserved; policy pending R-γ.)**

## 10. Review Disposition

**RATIFIED 2026-06-19** — authored by the Cowork PM; FULL independent DOCS-Project review returned **RATIFY-WITH-EDITS** (`nexsys-hivemind/context/audits/2026-06-19_AMD-94_DOCS_Review_Return.md`); Nick co-signed. **Folded into Locked Doc 15** (§3.4/§4.1/§5/§6/§13.4/§16/§8.1) with the two ratified edits applied — **E-1** (the boot invariant is discharged after a restore *only* by installing a fresh DEK version; the counter-proven-≥-priors branch is the crash-recovery path — closing the restore-half false-pass) and **E-2** (the §5/INV-02 discriminator wording made encoding-neutral so the additive-column placement R-γ may pick is not foreclosed); the optional **§8.1 `ScopeKeyManager` currency rider was KEPT** (it closes the §8.1-vs-§3.8 contradiction left by the AMD-86-E2 fold). `AMD-94-INV-01/02` registered (Architecture_Invariants_v1.md §48); watermark **AMD-93 → AMD-94** (165/48); §6 `[BLOCKING-for-M6-impl]` resolved; **OR-M6-NONCE restore-half CLOSED**; app-bootstrap **AB-4 un-gated** (the F1 envelope-finalization gate is satisfied — the slot is reserved, `v1` pinned). R-γ still refines the version *policy* (separate, later, non-blocking). This amendment was the sanctioned re-open -> review -> ratify vehicle for the A4 ruling.
