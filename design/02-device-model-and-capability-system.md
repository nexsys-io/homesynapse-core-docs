...
| QUANTITY | `QuantityValue(double value, String unit)` | object `{value, unit}` | Physical quantity normalized to the canonical unit at construction (hand-rolled, no JSR 385 — REC-93). Magnitude-comparable on the canonical `value`. (AMD-47) |
| ARRAY | `ArrayValue(List<AttributeValue> elements)` | array | Ordered, **full-replacement** list (no delta semantics — bounded-window-advancer compatible, REC-27). (AMD-47) |
| DEGRADED | `DegradedAttributeValue(String originalTypeName, String rawForm, String failureReason)` | object | Subtype-level upcast fallback paralleling `DegradedEvent` (REC-29). **Sentinel — not schema-declarable** (AMD-47-INV-04); never enters canonical state under strict mode. (AMD-47) |

The QUANTITY / ARRAY / DEGRADED rows were added by AMD-47 (AttributeValue Hierarchy Expansion + `AttributeValueUpcaster` SPI), RATIFIED 2026-05-30; the production types and the SPI are implemented by M4.B3. See `design/amendments/AMD-47_AttributeValue_Expansion_and_Upcaster.md`.
...
AttributeSchema {
    ...
    type:             AttributeType   -- BOOLEAN, INT, FLOAT, STRING, ENUM, QUANTITY, ARRAY (AMD-47; DEGRADED is sentinel-only, not schema-declarable — AMD-47-INV-04)
...
**Realization currency (M9.4a, 2026-07-04 — AMD-98 §2.2/§2.3):** this paragraph is now enforced code. Adoption installs the per-device tuning as a rebuilt per-entity `ConfirmationPolicy` + per-command `defaultTimeout` (re-derived at re-link from the cached profile match); `UNCONFIRMABLE` maps to `DISABLED` with the adapter publishing the immediate honest `command_result(outcome="unconfirmed", reason)` — the "reason recorded" clause lands adapter-side per INV-CE-04. Parameterized commands without declared outcomes confirm via ledger-side derivation from the commanded value + this capability's tolerance. AMD-97-INV-01 is loop-level-tested (`HeroLoopHardwareFreeIT`). No new mechanism — the Locked concept realized.
...
| `AttributeValue` | Sealed interface | Typed attribute value. `permits` enumerates exactly **8** variants (AMD-47-INV-01): `BooleanValue`, `IntValue`, `FloatValue`, `StringValue`, `EnumValue`, `QuantityValue`, `ArrayValue`, `DegradedAttributeValue`. The `AttributeValueUpcaster` SPI (REC-29) lives in the same package. Expanded by AMD-47 (RATIFIED 2026-05-30); types implemented by M4.B3 — see `design/amendments/AMD-47_AttributeValue_Expansion_and_Upcaster.md`. |
| `QuantityValue` | Record | `(double value, String unit)` pair for physical quantities — `value` is the canonical-normalized magnitude, `unit` the canonical unit symbol. Hand-rolled deterministic normalization at construction, **no JSR 385 / units library** (REC-93, AMD-47-INV-03). (AMD-47) |
| `ArrayValue` | Record | `(List<AttributeValue> elements)` — ordered, unmodifiable, null-free, possibly-empty list with **full-replacement** semantics (no element deltas — AMD-47-INV-05). (AMD-47) |
| `DegradedAttributeValue` | Record | `(String originalTypeName, String rawForm, String failureReason)` — subtype-level upcast fallback paralleling `DegradedEvent`; `attributeType()` → `DEGRADED` sentinel; never written to canonical state under strict mode (AMD-47-INV-04). (AMD-47) |
| `AttributeValueUpcaster` | Interface (SPI) | Migration seam for evolving stored `AttributeValue`s across type changes. Strict mode (core projections) halts on failed upcast; lenient mode (forensic/diagnostic) yields a `DegradedAttributeValue`. Constructor-injected, no `ServiceLoader` (DECIDE-04). (AMD-47, REC-29) |
...
This subsystem implements the `HealthContributor` interface (Doc 11 §8.1, §8.2) to report health state transitions to the HealthAggregator. The Device Model is classified as CORE_SERVICES tier (Doc 11 §7.1) with a 15-second startup grace period. Health state changes are reported via `HealthContributor.reportHealth(status, reason)` when the conditions in the table above transition.
