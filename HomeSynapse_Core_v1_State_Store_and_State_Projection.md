...
**Dependencies:** Event Model & Event Bus (§3.1 producer boundaries, §3.3 priority model, §3.4 subscription model, §3.6 backpressure and coalescing, §3.7 processing modes, §3.10 schema versioning, §4.1 event envelope, §4.2 domain event store schema, §4.3 event type taxonomy, §4.4 payload schemas), Device Model & Capability System (§3.3 entity structure, §3.4 capability model, §3.5 attribute type system, §3.12 enable/disable lifecycle, §3.13 device replacement, §6.6 registry rebuild, §7 interaction table, §8 key interfaces)
**Dependents:** Persistence Layer (checkpoint storage schema, checkpoint write mechanics), Automation Engine (state queries for trigger evaluation and condition checking), REST API (current state endpoints, entity listing), WebSocket API (state change push notifications), Observability & Debugging (state health indicators, staleness monitoring), Web UI (dashboard state rendering, entity history graphs)
...
1. **Should the State Store maintain state for entities that have been removed (via `device_removed`)?**
   The current design does not subscribe to `device_removed` because it is a device-subject event and entity removal is handled by the entity registry producing `entity_disabled` events. However, the Device Model does not explicitly document whether `device_removed` produces `entity_disabled` for each entity on the device. If it does, the State Store freezes their state naturally. If it does not, removed entities remain in the view with stale state indefinitely.
   Options: (a) Confirm with the Device Model that `device_removed` produces `entity_disabled` for all child entities, and the State Store handles it via existing logic. (b) Subscribe to `device_removed` and actively purge state for the device's entities. (c) Leave removed entities in the view with frozen state and rely on a future housekeeping mechanism to clean up.
   Needed: Clarification on the Device Model's `device_removed` behavior and whether it cascades to entity lifecycle events.
   Status: **[BLOCKING]** — the answer affects the subscription filter, the data model (should `EntityState` include a `removed` flag?), and the checkpoint content.
