...
1. **[RESOLVED] Should the State Store maintain state for entities that have been removed (via `device_removed`)?**
   Resolution: **Option (a) confirmed.** The Device Model (§3.12, Device Removal) explicitly documents that `device_removed` produces `entity_disabled` for each child entity before the device-level event. The State Store handles device removal entirely through the existing `entity_disabled` event processing — it freezes each entity's attribute state and adds the entity to the `disabledEntities` set. No subscription to `device_removed` is needed. No `removed` flag is needed on `EntityState`. Removed entities remain in the view with frozen state and `disabled` status until the next checkpoint rebuild or a future housekeeping mechanism purges them.
