...
2. **NO_DIRECT_FILESYSTEM_IN_CORE** — Core modules cannot use `java.io.File` or `java.nio.file.Files`. This means the core event model, device model, state store, automation, and configuration modules are already filesystem-agnostic. Only the persistence module touches the filesystem (via SQLite). A cloud persistence backend would not need to modify any core module.
   ...
   > **[STALENESS NOTE — 2026-06-10, Rule-5 ruling]:** As of ratified **AMD-71** and the M6.1a gate-fix (PM ruling 2026-06-10, committed `9035110`), **configuration is no longer filesystem-agnostic** — `com.homesynapse.config` is the *designated* filesystem consumer (YAML config tree + composed-schema cache), rooted exclusively at the composition-root-injected `configDir` `Path`, and was dropped from Rule 5's package list. The compensating control is AMD-71-INV-01 canonicalization-containment (`toRealPath()` under `integrations/`, pinned by `ConfigLayoutTest`). The cloud-enablement argument above survives in adjusted form: config's filesystem surface is narrow, injected, and containment-guarded — a cloud config backend would replace the injected root, not core code. Event/device/state/automation remain filesystem-agnostic; Rule 4 (NO_REVERSE_DEPENDENCIES) still lists config.
