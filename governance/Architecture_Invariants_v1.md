...
| `AC` | AIoT + Cloud Readiness (Doc 17) | §50 |
| `AMD-97-INV` | Device Confirmation Characterization (AMD-97) | §51 |
| `EXT` | Extension & Plugin Architecture (Doc 18) | §52 |
| `REG` | Device/Entity Registries as Projections of the Event Log (AMD-99) | §53 |

The §17 Invariant Index provides the canonical per-identifier lookup; the §18 Traceability Matrix maps each category to failure modes and opportunities. The BUS / PROJ / WRITER / SUB-ISO categories were added by Phase 3 governance work (AMD-41, AMD-42, AMD-43, applied 2026-05-16); their canonical definitions live in §19. The `AMD-47-INV-NN` identifiers are **amendment-scoped** contract-level invariants (the convention introduced by the projection block's `AMD-50-INV-NN`); their canonical definitions live in §20 and they trace 1:1 to AMD-47 (RATIFIED 2026-05-30). The `AMD-54-INV` … `AMD-64-INV` categories (§24–§34) were registered together at the Workstream C integration-block ratification (2026-06-05, single review return) and trace 1:1 to AMD-54..64. The `AMD-66-INV` … `AMD-71-INV` categories (§37–§41) were registered together at the M6 configuration-block ratification (2026-06-09, single review return) and trace 1:1 to AMD-66/67/68/70/71; **AMD-69 is DEFERRED (Tier-2/OQ-15-3) and registers no invariant — the number stays reserved.** The sections are numbered by AMD order (66, 67, 68, 70, 71 → §37–§41); the section count is five because the deferred AMD-69 contributes none. The **`SA`** category (§49) was registered at the **Doc 16 (Superior Automation Layer) Lock — 2026-06-20** — a **new design-doc Lock, not an amendment**, so (like the §19 Phase-3 subsystem categories BUS/PROJ/WRITER/SUB-ISO and the foundational docs) it mints a subsystem category at its own Lock and **the on-disk amendment watermark stays AMD-94**; INV-SA-01/02 are novel (registered first-class) and INV-SA-03/04 are citing compositions of existing parents. The **`AC`** category (§50) was registered at the **Doc 17 (AIoT + Cloud Readiness) Lock — 2026-06-26** — likewise a **new design-doc Lock, not an amendment** (the watermark is AMD-95 from the same-session AMD-95 ratification, not from this Lock); it mints **one** invariant, **AIOT-INV-1** (AI is never an autonomous actuator), a citing composition of INV-SA-04 + AMD-90-INV-01 + **INV-ES-09** (pure-function replay — D2's canonical id, registered 2026-07-01 under §2; the former "pending registration" parenthetical is resolved) that adds a new AI-actor-routing constraint its parents do not impose; the other two Doc 17 candidates (AIOT-INV-2/3) were kept as design principles. The **`AMD-97-INV`** category (§51) was registered at the **AMD-97 ratification (2026-07-01)** — amendment-scoped per the §20 convention; one invariant (AMD-97-INV-01, never-false-CONFIRMED). The **`EXT`** category (§52) was registered at the **Doc 18 (Extension & Plugin Architecture) Lock — 2026-07-03** — a **new design-doc Lock, not an amendment** (the §49/§50 precedent; **the on-disk amendment watermark stays AMD-97**); it mints **two** invariants, **EXT-INV-1** (core never depends on third-party extensions) and **EXT-INV-2** (extension capability is declared, not discovered) — both citing compositions that add a constraint their parents do not impose; EXT-INV-2 is minted UNSCOPED with its enforcement-arrival deferral stated inline in the §52 entry per the 2026-07-03 N-2 co-sign ruling. The **`REG`** category (§53) was registered at the **AMD-99 (Registries-Projection) ratification — 2026-07-08** — an amendment ratification, so **the on-disk amendment watermark advances AMD-98 → AMD-99 with it** (drift discriminators: invariant count 174→175, categories 52→53); it mints **one** invariant, **REG-INV-1** (the device/entity registries are projections of the event log), as a NAMED architecture category rather than the amendment-scoped `AMD-99-INV` form per the ratified AMD-99 §4 text — it states a permanent event-sourcing property of the platform (the registry-side completion of the §2 family), not an amendment-implementation contract; enforcement (the `REGISTRY_MUTATION_ONLY_VIA_PROJECTION` ArchUnit rule) arrives WITH the DUR coding WU per the ratified R-B — no deferral.
...
### INV-ES-09: Pure-Function Replay (registered 2026-07-01; decision-record D2, ratified 2026-06-25)

Log replay is a pure function over the event log: replaying the persisted log to rebuild projections, ledgers, or any in-memory state MUST produce **zero external side-effects**. Device dispatch, adapter/network I/O, notifications, and any action observable outside the log run **only** on new-command handling in **LIVE** mode — **never** during `REPLAY`. A side-effect on replay re-fires real-world commands (the canonical, safety-critical event-sourcing failure). The property is realized structurally at two layers: the event bus's `REPLAY → TRANSITION → LIVE` mode FSM **and** the per-subscriber guard (an effectful subscriber additionally verifies mode and does not act outside LIVE — `CommandDispatchSubscriberTest.onCommandIssued_inReplay_doesNotDispatch`), and it is **CI-pinned** by a composition-root seeded-log replay test asserting zero dispatch/run/confirmation side-effects (M7.4d `RunPipelineReplaySafetyTest`).

**Addressed failure mode:** recovery/restart replay physically re-executing past commands (lights re-fire, locks re-actuate) — the event-sourcing failure class the prior-art study flagged as safety-critical.

**Registration provenance:** ratified in the 2026-06-25 deeper-M7 decision record (D2, Nick co-signed), which explicitly deferred the canonical register entry to this review→ratify fold (watermark-neutral doc-currency registration per the SA/AC precedent — this entry moves no watermark). Empirical validation: the V2-A spike (1e6 replayed `command_issued` → zero dispatch) + the green M7.4d gate on core `4e31aed`+.

**Test:** `RunPipelineReplaySafetyTest` (composition-root: seed a log, replay from checkpoint 0, assert zero `command_dispatched`/`command_result`/run-init/confirmation side-effects, with a LIVE positive control proving non-vacuousness), run by `./gradlew check` → CI.
...
| **INV-ES-09** | Pure-Function Replay (zero external side-effects on log replay; D2) | §2 |
...
| **AIOT-INV-1** | AI Is Never an Autonomous Actuator (proposer-only; no AI code path actuates a device or mutates state outside the engine) | §50 |
| **AMD-97-INV-01** | Never a False CONFIRMED (a `DISABLED`/`UNCONFIRMABLE` command never renders `CONFIRMED`) | §51 |
| **EXT-INV-1** | Core Never Depends on Third-Party Extensions (boots, runs, and passes CI with the extension set empty) | §52 |
| **EXT-INV-2** | Extension Capability Is Declared, Not Discovered (the full surface is static manifest data validated before load) | §52 |
| **REG-INV-1** | The Device/Entity Registries Are Projections of the Event Log (single projection-apply path; boot rebuild by replay; write-ahead + idempotent-by-identity) | §53 |

**Total: 175 invariants across 53 identifier categories (§0.3).** _Regenerated from this table at the 2026-07-08 AMD-99 ratification (v25 hub): **+REG-INV-1** (§53 — the device/entity registries are projections of the event log; +1 category `REG`, a NAMED architecture category minted at an amendment ratification per the ratified AMD-99 §4/R-B text; **the on-disk amendment watermark advances AMD-98 → AMD-99**; enforcement — the `REGISTRY_MUTATION_ONLY_VIA_PROJECTION` ArchUnit rule — arrives WITH the DUR coding WU, no deferral). Prior regeneration at the 2026-07-04 AMD-98 ratification (v18 hub): **zero mints, counts unchanged 174/52; the on-disk amendment watermark advances AMD-97 → AMD-98** (the M9.4a consolidated amendment — EZSP band correction superseding AMD-96/E1's v14 acceptance in part + the confirmation-pipeline realizations; AMD-97-INV-01 gains its loop-level enforcing test). Prior regeneration at the 2026-07-03 Doc 18 Lock (v17 hub): **+EXT-INV-1/2** (§52 — Extension & Plugin Architecture; +1 category `EXT`; a **new-design-doc Lock** per the §49/§50 precedent — **the on-disk amendment watermark stays AMD-97**; EXT-INV-2 minted unscoped with its enforcement-arrival deferral inline per the N-2 co-sign ruling, pm-handoff beat-65). Prior regeneration 2026-07-01 consolidated governance pass (172/51: +INV-ES-09 §2, +AMD-97-INV-01 §51; the watermark advanced AMD-95 → AMD-96 → AMD-97 in that pass); 2026-06-26 Doc 17 Lock (170/50, §50, +1 — AIOT-INV-1; watermark AMD-95); 2026-06-20 Doc 16 Lock (§49, +4 — INV-SA-01..04; watermark stayed AMD-94); 2026-06-19 AMD-94 ratification (165/48); 2026-06-12 (163/47, M7 block); 2026-06-09 (152/41, M6 block). Never propagate a stated total — re-derive from this table._
...
| §50 AIoT + Cloud Readiness (Doc 17) | An AI subsystem actuating a device or mutating state outside the deterministic engine (the autonomous-misfire class the "planner→verifier→deterministic-executor" consensus exists to prevent) — i.e. AI reaching `ActionExecutor`/dispatch by any path other than a proposed definition/command the engine governs, statically verifies, and audits. |
| §52 Extension & Plugin Architecture (Doc 18) | A core feature silently taking a load-bearing dependency on extension-supplied capability — core no longer boots or passes CI with the extension set empty (the SmartThings/webCoRE collapse class); a third-party extension acquiring undeclared capability at runtime — dynamic discovery/registration bypassing the static, before-load-validated manifest (the trust boundary going paper-only). |
| §53 Registries as Projections of the Event Log (AMD-99) | Imperative in-memory registries as the ONE hole in "everything is provably derived from the log" (measured on the bench, iterations 3–5a: identity re-mint on every restart · orphan view rows · the `onRejoin`/pin-2 arm structurally unreachable across restarts · entity-ID-bound automations unable to survive the very boot that loads them); a second drift-prone source of truth bolted onto the moat (the rejected snapshot class — "the precise asterisk we don't want to ship"); silent regression via an unguarded mutation path outside the projection (the class the ArchUnit rule exists to prevent). |
...
Every event persisted to the WAL is delivered to each registered subscriber exactly once and in position order during normal operation (the order clause added by AMD-101, 2026-09-13). Duplicate delivery during crash recovery is bounded by the subscriber's last persisted checkpoint position (`CheckpointStore`) and is reconciled by subscriber idempotency (INV-ES-05). The event bus MUST use the per-subscriber checkpoint as the resume gate after a process restart, and the REPLAY → TRANSITION → LIVE transition MUST hand the subscriber's read-forward cursor to the drain, so that events delivered during catch-up are not re-delivered during drain; `lastReplayedPosition` remains REPLAY's own high-water mark (AMD-42 §3.4.2; AMD-101 §2). LIVE delivery is the read-forward from that cursor; the persisted checkpoint is the durable restart floor and no delivery decision reads it (AMD-101 §2; realized by BUS-ORDER-1, core `a458a64`).

### INV-BUS-02: Publish Is Non-Blocking on Backpressure

`EventPublisher.publish()` MUST NOT block on writer queue depth, semaphore acquisition, or any other depth-gated mechanism. Natural backpressure arises from the single-thread write executor (INV-WRITER-01, AMD-26): callers park on their handoff future, which completes only when the writer drains to their slot. Saturation manifests as elevated per-call latency, never as `publish()` hanging. **[Correction 2026-09-06 (v66 hub; source: the FIX-1 return 2026-09-05 [INFO] 5, verified against `app/homesynapse-app/src/test/java/com/homesynapse/app/HomeSynapseArchRules.java` at `093d5b4`):** the ArchUnit rule `EVENT_PUBLISHER_HAS_NO_DEPTH_GATED_LOCK` that AMD-43 / PLAN-M3-CONSOLIDATED-02 assigned to M3.3 was never written — the suite carries eleven rules (`NO_DIRECT_FILESYSTEM_IN_CORE` · `NO_DIRECT_TIME_ACCESS` · `NO_INTERNAL_PACKAGE_ACCESS` · `NO_JACKSON_IN_DOMAIN_MODEL` · `NO_JSON_TYPE_INFO_IN_EVENTS` · `NO_REVERSE_DEPENDENCIES` · `NO_SERVICE_LOADER` · `NO_SYNCHRONIZED_METHODS` · `QUERY_SERVICE_READ_ONLY` · `REGISTRY_MUTATION_ONLY_VIA_PROJECTION` · `REST_ENDPOINTS_NO_EVENT_PUBLISHING`) and no structural rule covers the publish path. The invariant is enforced BEHAVIORALLY by `EventBusContractTest.publishDoesNotBlockAt5000` (core/event-bus testFixtures) and by review. A structural rule remains a candidate; until one lands, the sentence that followed here ("no class in `core/persistence` or `core/event-bus` may import `Semaphore`/`Lock` or call `Object.wait()` on a path reachable from `publish()`") is the review rule, not a test.]**
...
## 50. AIoT + Cloud Readiness (Doc 17)

§50 registers the **INV-AC** invariant category introduced by **Doc 17 — AIoT + Cloud Readiness** (the reserved-architecture readiness doc: the immutable log as the universal substrate, cloud-replication outward, and the cross-cutting AI-safety frame), **Locked 2026-06-26**. Registered here (plus §0.3, the §17 Invariant Index, and the §18 Traceability Matrix) at the Doc 17 Lock, following the §49 (INV-SA) / §19 subsystem-category precedent — a **new design-doc Lock, not an amendment**, so it mints a subsystem category at its own Lock; **the on-disk amendment watermark is AMD-95** (from the same-session AMD-95 currency amendment, which itself minted zero invariants — this Lock does not move the watermark). The drift discriminators are the invariant count (169→170, 49→50 categories) and the Locked-doc set. Owner doc: **Doc 17** (Locked 2026-06-26). Independent DOCS review return: `nexsys-hivemind/context/audits/2026-06-26_Doc17_independent_DOCS_Review_Return.md` (SCOPE = new-doc cut RIGHT, conditional on S1 — folded; DOCUMENT = RATIFY-WITH-EDITS, all NON-BLOCKING; S1/S2 + E1–E5 folded; all 11 cited invariant ids source-verified). Of the three Doc 17 candidates, **one is minted (AIOT-INV-1)**; **AIOT-INV-2/3 are kept as design principles** (AIOT-INV-2 is a near-duplicate of INV-LF-02 + INV-SA-02; AIOT-INV-3 is subsumed by INV-SA-03). This doc adds **no sealed permit, no event type, and no source change**; the structural enforcement (E3 proposer-only port) + the AI-authoring milestone land post-MVP.

### AIOT-INV-1: AI Is Never an Autonomous Actuator

No AI code path actuates a device or mutates state outside the deterministic engine; every AI-originated effect enters as a **proposed automation definition or proposed command** through an inbound proposer-only port and passes through the engine's governance (expansion into the sealed permits, condition/mode/cascade evaluation), the **no-autonomous-retry** contract, and the **pure-function-replay** path before any real-world command issues. A future AI module is wired at the composition root as a **proposer-only adapter** with no actuation capability and no outbound device/dispatch dependency (mirroring INV-LF-02's structural denial of outbound network capability), making the invariant CI-testable on two axes — temporal (no side-effect on replay) and actor (the AI actor structurally cannot reach `ActionExecutor`/dispatch except via a governed proposal). *(Citing composition of **INV-SA-04** (running automations degrade deterministically; engine never autonomously re-issues) + **AMD-90-INV-01** (confirmation never blocks/retries) + **INV-ES-09** (pure-function replay — D2's canonical id, registered 2026-07-01, §2; enforcing test `RunPipelineReplaySafetyTest`, M7.4d) — strengthens them with a new constraint the parents do not impose: binding a **new actor class (AI)** to proposer-only routing. Doc 17 §2.3, §3.3, §6; the V2-C AIoT-safety-frame research.)*

---

## 51. Device Confirmation Characterization (AMD-97)

§51 registers the **`AMD-97-INV`** amendment-scoped category, minted at the **AMD-97 ratification (2026-07-01)** — the first-class device-level confirmation characterization (the Doc 08 §3.6 `DeviceProfile` `confirmation[]` block + the Doc 02 §3.8 per-device confirmability override). Amendment file: `design/amendments/AMD-97_Device-Confirmation-Characterization.md`. Review chain: the 2026-06-28 governance dispatch (v11 hub, from the Stream-B research return §4) → independent DOCS review (RATIFY-WITH-EDITS) → the v13 hub's independent re-derivation of every edit against the Locked text → Nick's co-sign 2026-07-01, with the measured bench values folded (nexsys-bench `5ceff3b`). The watermark advanced **AMD-96 → AMD-97** at this ratification (drift discriminators: invariant count 171→172 within this pass's 170→172, categories 50→51).

### AMD-97-INV-01: Never a False CONFIRMED

A command whose confirmation characterization is `DISABLED`/`UNCONFIRMABLE` **never renders `CONFIRMED`** — the honest verdict is structural, not best-effort. An `UNCONFIRMABLE` path renders `UNCONFIRMED` immediately with the reason recorded; a `BEST_EFFORT` path renders `CONFIRMED` only on genuine authoritative evidence (report or explicit readback), never on ACK alone (measured: the Wave-1 light ACKs SUCCESS on write-only paths that never report — a false-CONFIRMED-on-ACK engine would lie on real silicon). *(Composes with **AMD-90-INV-01** (per-action, no engine retry), **AMD-95** (`DISABLED` ≡ optimistic — now with a recorded honest reason), **AMD-87-INV-01** (Expectation codec round-trip), **INV-SA-03** (never-silent degrade); adds the device-level honesty constraint none of the parents impose. Read-path half enforced structurally today (M7.5a registry-free derivation: a `DISABLED` command is never ledger-tracked → no `state_confirmed` → no `CONFIRMED`); device-level half pinned by M9's confirmation-acceptance tests on the captured fixtures (`nexsys-bench/fixtures/`). Doc 08 §3.6; Doc 02 §3.8; bench E5.)*

**Test:** M9 confirmation acceptance — replay the Hue confirmation-windows fixture: assert `on_off`/`brightness`/`color_temperature` commands reach `CONFIRMED` only via authoritative `Report_Attributes`; assert the `color_loop_set` (ACK-SUCCESS, never-reported) and `identify` paths render `UNCONFIRMED` immediately and never transition to `CONFIRMED`; assert an expectation superseded by a newer same-attribute command expires without a false verdict.

---

## 52. Extension & Plugin Architecture (Doc 18)

§52 registers the **`EXT`** invariant category introduced by **Doc 18 — Extension & Plugin Architecture** (the reserved-architecture readiness doc for the third-party extension seam class: seven seams reserved as §4 non-preclusion rows, publisher-scoped namespace/identity governance in §3.5, the tiered-trust ladder, and the no-sandbox-claims honesty rule as doc text), **Locked 2026-07-03**. Registered here (plus §0.3, the §17 Invariant Index, and the §18 Traceability Matrix) at the Doc 18 Lock, following the §49 (INV-SA) / §50 (AIOT) precedent — a **new design-doc Lock, not an amendment**, so **the on-disk amendment watermark stays AMD-97**; the drift discriminators are the invariant count (172→174, 51→52 categories) and the Locked-doc set. Owner doc: **Doc 18** (Locked 2026-07-03). Independent DOCS review return: `nexsys-hivemind/context/audits/2026-07-03_Doc18_independent_DOCS_Review_Return.md` (RATIFY-WITH-EDITS; B-1 — the near-duplicate completion adding INV-CE-05/INV-SE-04 as fourth parents, both distinctness arguments hub-re-verified against this register — plus N-1..N-4, all folded pre-Lock). Co-sign record: `nexsys-hivemind/context/handoff/pm-handoff.md` beat-65 (2026-07-03, verbatim) — **N-2 ruled: EXT-INV-2 is minted UNSCOPED with the enforcement-arrival deferral stated INLINE in this entry** (the anti-paper-invariant condition; the AIOT-INV-1 precedent — a deferred-enforcement mint is honest only when the deferral is named and scheduled in the register itself). Both invariants are citing compositions that add a constraint their parents do not impose. This doc builds nothing; the ladder rungs that make EXT-INV-2's second half enforceable are tracked as the EXT-LADDER (assessment R-4) backlog rows.

### EXT-INV-1: Core Never Depends on Third-Party Extensions

HomeSynapse Core boots, runs, and passes CI with the third-party extension set empty; no core feature may take a load-bearing dependency on any extension-supplied capability, type, or event. *(Citing composition of **INV-RF-01** (integration crash isolation — binds failure propagation) + **INV-LF-02** (no outbound dependence — binds the network direction) + **LTD-17** (static composition — binds the build graph) + **INV-CE-05** (extension model with stability guarantees — binds isolation, quotas, independent version pinning, graceful degradation on extension failure); adds the reverse-direction constraint none of the parents impose: INV-CE-05 requires a *failing* extension to degrade gracefully, but no parent requires core to boot and pass CI with the extension set EMPTY — the load-bearing-ness of the third-party actor class as such (the SmartThings/webCoRE collapse class; Doc 18 §6, dossier L-1/L-13/L-27 vs the HA counter-shape L-17/L-18). CI-testable now: the composition root builds and boots with zero non-first-party integrations — the M9.1 fake-adapter path already approximates the harness.)*

### EXT-INV-2: Extension Capability Is Declared, Not Discovered

Every third-party extension declares its full capability surface — event classes published, operations exposed, services required, config schema version, isolation level — in static manifest data validated before load; no capability is granted at runtime that was not declared. *(Citing composition of the frozen **`IntegrationDescriptor`** (AMD-54..64: requiredServices, dataPaths, configSchemaMajor/Minor, isolationLevel) + the **M3.6c per-module event-class manifests** + **AMD-55-INV-01** (default-method backward-compatibility — a capability's absence is truthfully declared, never silently claimed) + **INV-SE-04** (least privilege for integrations); adds the static-declaration-completeness-before-load constraint neither nearest parent imposes — INV-SE-04 binds the privilege *ceiling* (may not exceed granted permissions), EXT-INV-2 binds declaration *completeness* (the full surface is static manifest data validated before load; nothing discovered or registered at runtime) — the two are orthogonal. Doc 18 §6; L-5/L-7 — Chrome's manifest-as-trust-contract and remote-code ban; VS Code's declarative contribution points.)* **Enforcement arrival (stated inline per the N-2 co-sign ruling, 2026-07-03):** the CI-testable core today is the descriptor's **before-load declaration/validation**; **the second half — no undeclared runtime capability — becomes CI-testable at ladder rung 2** (permitted-publish-types enforcement + the manifest-vs-behavior test; tracked as the EXT-LADDER/R-4 rows in `nexsys-hivemind/context/planning/phase-3-milestone-backlog.md`, which also carry the F-1 `IntegrationContext`-javadoc reconciliation). Minted unscoped with the deferral named and scheduled here, per the AIOT-INV-1 precedent (its E3 actor-axis test likewise lands post-MVP).

---

## 53. Device/Entity Registries as Projections of the Event Log (AMD-99)

§53 registers the **`REG`** invariant category introduced by **AMD-99 — Registries-Projection** (`design/amendments/AMD-99_Registries-Projection.md`), **RATIFIED 2026-07-08** (Nick: *"ratify as recommended — (A), and R-B through R-E as written"*; his full assessment is preserved verbatim in `nexsys-hivemind/context/handoff/pm-handoff.md` v25 beat-1). Registered here (plus §0.3, the §17 Invariant Index, and the §18 Traceability Matrix) at the ratification; **the on-disk amendment watermark advances AMD-98 → AMD-99** (drift discriminators: invariant count 174→175, categories 52→53). REG is minted as a NAMED category rather than the amendment-scoped `AMD-99-INV` form per the ratified §4 text — it states a permanent event-sourcing property (the registry-side completion of the §2 family: the one place "everything is provably derived from the log" had a hole, measured on the bench at iterations 3–5a), not an amendment-implementation contract. Ruling of record: pm-handoff v24 beat-4 (Nick, verbatim — *"DUR — (i). Event-source the registries"* + guardrails G1/G2). The governing decision criterion (Nick's, binding): the registries are reconstructable from the log ALONE, as a pure function of replay — payload option (B) was REJECTED because its rebuild depended on non-log artifacts (`zigbee-devices.json` + profile files). Implementing WU: the DUR coding WU (authored against the ratified AMD-99 text; it carries the event mints — EventTypes 71→73 · @EventType 41→43 · codecs 53→55 · `EventCategoryMapping` +2 · count-pins lockstep — and the enforcement).

### REG-INV-1: The Device/Entity Registries Are Projections of the Event Log

The device and entity registries are projections of the event log. Every registry mutation flows through a single projection-apply function whose only inputs are the registration/removal event types; at boot the registries are reconstructed by replaying those events; no other code path mutates registry state. Adoption-time writes use the SAME apply function (write-ahead: the event is durable before the in-memory apply; apply is idempotent by identity, so live bus delivery of a self-published event is a no-op). *(Verbatim from AMD-99 §4. Composes with **INV-ES-02** (state is always derivable from events — the registries ARE state), **INV-ES-04** (write-ahead persistence), **INV-ES-05** (at-least-once delivery with subscriber idempotency — the idempotent-by-identity clause is its registry-side instance), and **INV-ES-09** (pure-function replay) — completing the family on the registry side with a constraint none of the parents impose: registry MUTATION is structurally confined to the projection-apply path. The write-ahead + idempotent-by-identity clause is what keeps adoption synchronous — the event is durable, applied inline, and the later bus delivery is a no-op, so entities are immediately usable with no async gap at adoption. Registration updates ride an idempotent re-emit of the existing two event types — no third type (AMD-99 §3, fold F1). `device_removed` is the first-class tombstone the projection honors from day one, replay and live alike, though nothing emits it in production yet — the DUR WU proves the mechanism by test (AMD-99 §6).)* **Enforcement (arriving WITH the mechanism per the ratified R-B — no deferral):** the ArchUnit rule `REGISTRY_MUTATION_ONLY_VIA_PROJECTION` in `HomeSynapseArchRules` (app test classpath — production-code reach covers every module; the corrected-2026-06-13 reach caveat concerns non-app TEST code only) lands in the DUR coding WU itself and rides `./gradlew check` + CI — the gate of record — from day one.
